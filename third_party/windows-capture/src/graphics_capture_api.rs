...
// VENDOR PATCH (screen-mirror-app): Direct3D11CaptureFramePool buffer count.
//
// Upstream creates (and recreates) the frame pool with a SINGLE buffer. With a size-1
// pool the WGC surface for frame N is only returned to the pool when the synchronous
// `FrameArrived` handler RETURNS — so frame N+1 cannot be presented until the whole
// wall-clock cost of the user callback has elapsed. That serializes callback latency
// into the inter-frame interval; on a ~60fps iGPU, once the callback nears ~16.6ms the
// capture cadence collapses to a 30fps judder lock (frame_delta_p99 ≈ 33ms).
//
// A double-buffered pool lets WGC present the next surface while frame N is still being
// serviced, so callback latency no longer adds to the frame spacing. See engram bugfix
// #1232 (sdd/qsv-igpu-pipeline-perf/capture-rootcause).
const NUMBER_OF_BUFFERS: i32 = 2;
...
impl GraphicsCaptureApi {
    ...
    pub fn new<T: GraphicsCaptureApiHandler<Error = E> + Send + 'static, E: Send + Sync + 'static>(
        ...
        // Create frame pool
        let frame_pool =
            Direct3D11CaptureFramePool::Create(&direct3d_device, pixel_format, NUMBER_OF_BUFFERS, item.Size()?)?;
